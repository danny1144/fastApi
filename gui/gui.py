...
sg.theme('Purple')
label = [[sg.Image(r'tianqi.png', key='backBox')],
         [sg.Text('请输入城市、乡镇、街道、景点名称查询天气', size=(33, 0)),
          sg.InputText(size=(20, 0))],
         [sg.Button('确认', pad=((120, 0), 20)),
          sg.Button('退出', pad=((150, 0), 20))],
         [sg.Text('请选择查询区域')],
...
def get_city(name):
    r = requests.get(
        'http://toy1.weather.com.cn/search?cityname={}'.format(name), headers=header)
    ...
    if r.text != '<!-- empty -->':
        ...
        for element in elements:
            ...
            if len(min_tem):
                ...
                min_tem = '无'
            wind_direction = '、'.join(element.xpath(
                './p[@class="win"]/em/span/@title'))
            wind_scale = element.xpath('./p[@class="win"]/i/text()')[0]
            data.append([date, weather, max_tem, min_tem,
        ...
        for i in range(1, len(elements)-1):
            ...
            weather = elements[i].xpath(
                './p[@class="weather-info"]/text()')[0].strip()
            wind_direction = '、'.join(elements[i].xpath(
                './div[@class="wind-container"]/i/@title'))
            wind_scale = elements[i].xpath(
                './p[@class="wind-info"]/text()')[0].strip()
